			...
			if (sel == 1) {
				...
				if (choice == 0) {
					...
					while (true) {
						...
						System.out.print("삭제 수량 입력 : ");
						int number = sc.nextInt();
						if (number < 0) {
							System.out.println("0이상의 숫자를 입력하세요");
							continue;
						}
						if (number > index[idx3]) {
							System.out.println("삭제 수량이 더 큽니다.");
							continue;
						}
						int idx4 = 0;
						int cnt = 0;						
						for (int i = 0; i < count; i++) {
							...
							if (cartList[idx4] == null) {
								for (int j = idx4; j < count - 1; j++) {							
						...
						for (int i = 1; i <= cnt; i++) {
							cartList[count - i] = new String[2];
						}
						count -= cnt;
						System.out.println(Arrays.deepToString(cartList));
						System.out.printf("%s %d개 삭제 완료%n", item1, cnt);
						break;
					} 
